...
def analyze(req: SymptomsRequest):
    ...
    if local_emergency_score >= EMERGENCY_THRESHOLD and not result["isEmergency"]:
        ...
        result["emergencyMessage"] = get_emergency_popup_message(result["detectedLanguage"], script)
        result["emergencyPopupMessage"] = get_emergency_popup_message(result["detectedLanguage"], script)
        if not result["firstAidInstructions"]:
            result["firstAidInstructions"] = get_first_aid(raw_text, result["detectedLanguage"], script)
...
def _analyze_locally(req: "SymptomsRequest", raw_text: str) -> dict:
    ...
    if not matched_symptoms:
        ...
        if category:
            reply = get_template(f"small_talk_{category}", lang, script=script)
        else:
            reply = get_template("no_symptoms", lang, script=script)
        return {
            "predictions": [],
            "severity": "",
            "isEmergency": False,
    ...
    # A predicted disease that is inherently emergency-level (per its own
    # metadata, e.g. Meningitis/Appendicitis/Heart Attack) only counts as
    # a real emergency signal if at least 2 of THAT disease's own defining
    # symptoms were actually mentioned - not just that it narrowly won the
    # classifier vote off a single generic, widely-shared symptom (e.g. a
    # bare "headache" should never alone imply Meningitis or Stroke).
    top_disease_is_emergency = False
    if predictions:
        top_meta = predictor.meta.get(predictions[0]["name"], {})
        if top_meta.get("severity") == "emergency":
            disease_words = " ".join(top_meta.get("symptoms", [])).lower()
            overlap = sum(1 for s in matched_symptoms if s.lower() in disease_words)
            top_disease_is_emergency = overlap >= 2
    ...
    if is_emergency:
        emergency_msg = get_emergency_popup_message(lang, script)
        first_aid = get_first_aid(raw_text + " " + english_text, lang, script)
        analysis_intro = get_template("emergency_intro", lang, script=script)
    else:
        emergency_msg = ""
        first_aid = []
        analysis_intro = get_template("analysis_intro", lang, script=script)
    ...
    return {
        ...
        "emergencyPopupMessage": get_emergency_popup_message(lang, script) if is_emergency else "",
